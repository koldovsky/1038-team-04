...
class gallery_sl {
   ...
   * @param {String} prefix
   */
  constructor(el, config = {}, prefix = 'gallery_sl-') {
    this.#state = {
      prefix,
      el,
  ...
  static getOrCreateInstance(target, config = {}, prefix = 'gallery_sl-') {
  ...
  static createInstances() {
    document.querySelectorAll('[data-slider="gallery_sl"]').forEach((el) => {
      const { dataset } = el;
      const params = {};
  ...
  #onClick(e) {
    ...
    if (!(e.target.closest('.gallery_sl-btn') || e.target.closest('.gallery_sl-indicators'))) {
      return;
    }
